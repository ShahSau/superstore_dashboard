...
const RegionFilters = ({ columnFilters, setColumnFilters, innerData, outerData, filterData }: { columnFilters: any[], setColumnFilters: any, innerData:any[], outerData:any[], filterData:any[] }) => {
        
    {/*outer is state list and inner is city list*/}
    const stateName = columnFilters.find((f) => f.id === "State")?.value || ""; 
    const taskNameCity = columnFilters.find((f) => f.id === "City")?.value || "";
        ...
        <div>
            <div className="p-10  z-0">
                <div className="max-w-md mx-auto">
                    <label htmlFor="select" className="font-semibold block py-2">State</label>
                ...
                {/*city */}
                <div className="max-w-md mx-auto">
                    <label htmlFor="select" className="font-semibold block py-2">City</label>
                    ...
                    <div className="relative">
                        ...
                        <input
                            ...
                            >
                                ...
                                >
                                    {
                                        stateName && filterData[stateName].map((d:string, index:number) => (
                                            <option key={index} value={d}>{d}</option>
                                        ))
                                    }
                                    {
                                        !stateName &&innerData.map((d:string, index:number) => (
                                            <option key={index} value={d}>{d}</option>
                                        ))
                                    }
                                </select>
